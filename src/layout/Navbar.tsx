...
export default function Navbar() {
  ...
  const navbar = NAVBAR_CONFIG.map((item, index) => (
    <Link key={`${item.title}-${index}`} href={item.path} className="min-w-10">
      <NavbarIconContainer
  ...
  return (
    <div className="fixed bottom-0 w-full">
      <div className="flex h-16 w-full items-center justify-center rounded-t-2xl bg-primary-ui bg-opacity-0 px-8 py-4">
