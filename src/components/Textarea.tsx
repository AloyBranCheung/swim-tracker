...
export default function Textarea({ name, label, className, labelClassName, onChange, value }: TextareaProps) {
  return (
    <div className="relative">
      ...
      <label className={twMerge("transition-all absolute left-3 top-2 z-10 text-sm font-medium text-secondary-font peer-placeholder-shown:text-lg peer-placeholder-shown:top-4 peer-focus:top-2 peer-focus:left-3 peer-focus:text-sm", labelClassName)} htmlFor="textarea">{label}</label>
    </div>
  );
}
